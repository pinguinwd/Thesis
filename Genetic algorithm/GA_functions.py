...
def evaluate_fitness_control(individual, inputs, outputs, input_cells, output_cells, steps, distance_method):
        # Use CuPy arrays for GPU computation
    individual = cp.array(individual)
    fitness_scores = []

    # Iterate over paired elements from the inputs and outputs lists
    for input_value, expected_output in zip(inputs, outputs):
        initial_sequence = cp.zeros(len(individual), dtype=cp.int32)
        input_binary = format(input_value, '0' + str(len(input_cells)) + 'b')

        # Set the specific cells in the initial sequence based on the binary input
        for idx, bit in zip(input_cells, input_binary):
            initial_sequence[idx] = int(bit)

        # Function to evolve the cellular automata, assumed to be defined elsewhere
        final_sequence = evolve_cellular_automata_gpu(initial_sequence, individual, steps, len(input_cells))

        # Convert the output of cellular automata to binary format
        output_binary = ''.join(str(final_sequence[idx]) for idx in output_cells)
        predicted_output = int(output_binary, 2)

        distance = abs(predicted_output - expected_output)

        # Calculate fitness as the inverse of the distance
        fitness_score = 1 / (1 + distance)
        fitness_scores.append(fitness_score)

    return np.mean(fitness_scores)

def evaluate_fitness(individual, inputs, outputs, input_cells, output_cells, steps, distance_method):
    # Use CuPy arrays for GPU computation
    individual = cp.array(individual)
    fitness_scores = []
    ...
    for input_value, expected_output in zip(inputs, outputs):
        ...
        # Calculate distance (assumed to have GPU or other efficient implementation)
        distance = calculate_distance(output_binary, expected_output_binary, distance_method)
